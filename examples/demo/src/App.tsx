...
const initialItems: Item[] = [
  {
    ...
    kind: 'card',
    name: 'Pricing card',
    frame: { x: 96, y: 96, width: 260, height: 190, rotation: 0 },
  },
  {
    id: 'image',
    kind: 'image',
    name: 'Cover image',
    frame: { x: 440, y: 150, width: 220, height: 220, rotation: 12 },
  },
  {
    id: 'note',
    kind: 'note',
    name: 'Sticky note',
    frame: { x: 200, y: 360, width: 240, height: 130, rotation: -6 },
  },
];
  ...
  return (
    <div className="layer__body">
      ...
      <input
        ...
        placeholder="Type here, no drag starts"
        aria-label="Example input inside a frame"
      />
...
function Layer({
  ...
  const f = useFrame({
    ...
    constraints: {
      minWidth: 48,
      minHeight: 48,
      ...(lockAspect ? { aspectRatio: 'preserve' as const } : {}),
    },
    ...(snap ? { snap } : {}),
    label: item.name,
  });
  ...
  if (live?.kind === 'resize')
    badge = `${Math.round(live.frame.width)} × ${Math.round(live.frame.height)}`;
  ...
  return (
    ...
    >
      ...
      {badge && (
        <span
          className="badge"
          style={{ transform: `translateX(-50%) rotate(${-item.frame.rotation}deg)` }}
        >
          {badge}
        </span>
      )}
    ...
    <div className="app">
      <header className="topbar">
        ...
        <div className="topbar__actions">
          ...
          <a
            className="btn btn--icon"
            href="https://github.com/KevinArce98/frameable"
      ...
      <aside className="sidebar">
        <section className="section">
          ...
          <Toggle
            label="Snap to grid"
            hint="24 unit grid, position and size"
            checked={grid}
            onChange={setGrid}
          />
          <Toggle
          ...
          {current ? (
            <div className="fields">
              <NumberField
                label="X"
                name="x"
                value={current.frame.x}
                onChange={x => updateCurrent({ x })}
              />
              <NumberField
                label="Y"
                name="y"
                value={current.frame.y}
                onChange={y => updateCurrent({ y })}
              />
              <NumberField
                label="W"
                name="width"
      ...
      <main
        className="stage"
        style={cssZoom ? { zoom: 0.6 } : undefined}
        onWheel={onWheel}
      ...
      >
        ...
        <footer className="statusbar">
          <span>
            pointer{' '}
            <strong>{pointer ? `${Math.round(pointer.x)}, ${Math.round(pointer.y)}` : '—'}</strong>
          </span>
          <span>
