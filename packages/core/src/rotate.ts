...
export function rotate(frame: Frame, options: RotateOptions): Frame {
  ...
  if (options.step && options.step > 0) rotation = Math.round(rotation / options.step) * options.step
  return { ...frame, rotation: normalizeAngle(rotation) }
}

export function rotationFromPointer(initial: Frame, start: Point, current: Point, step?: number): Frame {
