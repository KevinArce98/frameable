...
export function runSnappers(frame: Frame, snappers: Snapper | Snapper[] | undefined, ctx: SnapContext): SnapResult {
...
export function snapToGrid(size: number | Point): Snapper {
  ...
  return (candidate, ctx) => {
    if (ctx.kind === 'move') {
      return { frame: { ...candidate, x: roundTo(candidate.x, grid.x), y: roundTo(candidate.y, grid.y) }, guides: [] }
    }
    if (ctx.kind === 'resize') {
      return {
        frame: {
