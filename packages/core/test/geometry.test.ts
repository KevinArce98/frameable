...
import { aabb, applyBounds, corners, frameDelta, move, normalizeAngle, pointerAngle, rotate, rotationFromPointer } from '../src'
...
describe('angles', () => {
  ...
  it('rotates by the pointer sweep around the center', () => {
    ...
    expect(rotationFromPointer({ ...frame, rotation: 30 }, start, current).rotation).toBeCloseTo(120)
  })
})
