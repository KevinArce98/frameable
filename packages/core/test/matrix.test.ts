...
import { fromViewport, identity, invert, multiply, rotationMatrix, runSnappers, screenDeltaToSurface, screenToSurface, snapToGrid, surfaceToScreen, toStyle } from '../src'
...
describe('snapping', () => {
  ...
  it('snaps position on move and size on resize', () => {
    ...
    const resized = runSnappers(frame, [snapToGrid(8)], { kind: 'resize', handle: 'se', threshold: 4 })
    expect(resized.frame).toEqual({ ...frame, width: 104, height: 48 })
  })

  it('does nothing for rotation and without snappers', () => {
    expect(runSnappers(frame, snapToGrid(8), { kind: 'rotate', threshold: 4 }).frame).toEqual(frame)
...
describe('styles', () => {
  it('produces absolute positioning with rotation', () => {
    ...
    expect(toStyle({ x: 1, y: 2, width: 3, height: 4, rotation: 15 }).transform).toBe('rotate(15deg)')
  })
})
