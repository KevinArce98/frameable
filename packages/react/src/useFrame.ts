...
const DEFAULT_INTERACTIVE = 'input, textarea, select, button, a, [contenteditable], [data-frameable-ignore]'
...
function readModifiers(event: { shiftKey: boolean; altKey: boolean; metaKey: boolean; ctrlKey: boolean }): Modifiers {
...
export function useFrame(options: UseFrameOptions): UseFrameResult {
  ...
  const compute = useCallback((s: Session, point: Point, modifiers: Modifiers): { frame: Frame; guides: Guide[] } => {
    const { constraints, snap, snapThreshold, modifiers: modifiersEnabled = true } = latest.current
    const mods = modifiersEnabled ? modifiers : { shift: false, alt: false, meta: false, ctrl: false }
  ...
  const finish = useCallback(
    (phase: 'end' | 'cancel') => {
      ...
      const modifiers = s.latest?.modifiers ?? { shift: false, alt: false, meta: false, ctrl: false }
      const frame = phase === 'cancel' || !s.latest ? s.initial : compute(s, s.latest.point, modifiers).frame
  ...
  const begin = useCallback(
    (kind: Operation, event: PointerEvent<Element>, handle?: Handle) => {
      ...
      if (kind === 'move') {
        const selector = interactiveSelector === undefined ? DEFAULT_INTERACTIVE : interactiveSelector
      ...
      const focusTarget = kind === 'move' ? event.currentTarget : event.currentTarget.closest('[data-frameable="move"]')
      if (focusTarget instanceof HTMLElement && focusTarget.tabIndex >= 0) focusTarget.focus({ preventScroll: true })
      const s: Session = {
        id: nextId(),
        kind,
        ...(handle ? { handle } : {}),
  ...
  const getHandleProps = useCallback((handle: Handle) => pointerProps('resize', handle), [pointerProps])
  ...
  const keyboardTransaction = useCallback(
    (kind: Operation, next: Frame, modifiers: Modifiers) => {
      ...
      const bounded = kind === 'rotate' ? snapped.frame : applyBounds(snapped.frame, constraints?.bounds)
      const id = nextId()
  ...
  const onKeyDown = useCallback(
    (event: KeyboardEvent<Element>) => {
      ...
      if ((modifiers.meta || modifiers.ctrl) && (event.key === '[' || event.key === ']')) {
        ...
        keyboardTransaction('rotate', rotate(frame, { to: frame.rotation + direction * step }), modifiers)
      }
    },
    [keyboardTransaction],
  )
  ...
  return useMemo(
    ...
    [options.frame, active, transaction, guides, getDragProps, getHandleProps, getRotateProps, getKeyboardProps],
  )
}
